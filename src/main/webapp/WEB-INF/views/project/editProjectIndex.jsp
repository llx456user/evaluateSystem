...
<body>
	...
	<div class="panel" style="margin-bottom: 5px;" id="multiDroppableContainer">
	    ...
	    <div class="panel-body">
			...
			<button class="btn btn-primary" type="button" onclick="calc();">计算</button>
	...
	function save(){
		...
		$.ajax({
			url:ctx + '/project/saveProjectIndex',
			data:{reqJson:JSON.stringify(param)},
			dataType:'JSON',
			success:function(data){
				if(data.status==1){
	...
	　　}
		...
		　　for(var key in xArray[i]){
		    if(nCount<i){
                var  iptId = i+'_'+nCount ;
                td.push('<td><input type="text" readonly="readonly" name="ipt" class="form-control"  value="'  + xArray[i][key] + '"  id="' + iptId + '"/></td>');
			}else if(nCount==i){
                td.push('<td><input type="text" readonly="readonly" name="ipt" class="form-control" value="1"/></td>');
			}else{
		        var  iptId = i+'_'+nCount ;
                td.push('<td><input type="text" name="ipt"  onchange="changeArrayValue(this.id)" class="form-control" value="'  + xArray[i][key] + '"  id="' + iptId + '"/></td>');
			}
              nCount++;
		　　}
		...
		$("#table").html(thHtml + html.join('') + weightHtml.join(''));
	...
	function  changeArrayValue(id) {
	    var disIdArray = id.split("_");
	    var disId = disIdArray[1]+"_"+disIdArray[0];
        var value = $("#"+id).val();
		var dValue = '';
		if(value.indexOf("/") >= 0){
           ...
           if(valueArray[0]==1){
               ...
               dValue=valueArray[1]+"/"+valueArray[0];
		   }
