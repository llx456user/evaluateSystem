...
function init() {
    ...
    var partChildContextMenu =         //子节点右键信息
        $(go.Adornment, "Vertical",
            makeButton("add input",
            function (e, obj) {
                var contextmenu = obj.part;
                var part = contextmenu.adornedPart;
                var colname = "input" + (part.data.inServices.length + 1);
                var model = myDiagram.model;
                var data = obj.data;
                myDiagram.startTransaction("add sport");
                var color = connectPoint;
            ...
            makeButton("add out",
                function (e, obj) {
                    var contextmenu = obj.part;
                    var part = contextmenu.adornedPart;
                    var colname = "output" + (part.data.outServices.length + 1);
                    var model = myDiagram.model;
                    var data = obj.data;
                    myDiagram.startTransaction("add sport");
                    var color = connectPoint;
                    ...
                    myDiagram.model.insertArrayItem(part.data.outServices, -1, port);
                    myDiagram.commitTransaction("add sport");
                }),
            makeButton("Delete",
                function (e, obj) {
...
// To simplify this code we define a function for creating a context menu button:
    ...
    myDiagram.nodeTemplateMap.add("6",                 //结束节点 指标值
        $(go.Node, "Spot", nodeStyle(),
            $(go.Panel, "Auto",
                $(go.Shape, "Circle",
                    {
    ...
    myDiagram.nodeTemplateMap.add("8",      //子节点
        $(go.Node, "Spot",
            {
                selectionAdorned: false
            },
            ...
            $(go.Panel, "Auto", {name: "BODY"},
                $(go.Shape, "Cube1",  //model图形形状
                    {stroke: "gray", strokeWidth: 2, fill: childShapeBody},
                    new go.Binding("fill", "color"),//设置填充色
                    new go.Binding("stroke", "isSelected", function (b) {
                        return b ? SelectedBrush : UnselectedBrush;
                    }).ofObject()),
            ...
            {
                contextMenu: partChildContextMenu
            }
        ));
    // myDiagram.nodeTemplateMap.add("8",        //子节点
    //     $(go.Node, "Spot", /*clickModel(),*/
...
function save() {
    ...
    var visibleChild = $("#visibleChild").prop('checked');
    var visibleChildValue =  0 ;
    if(visibleChild){
        visibleChildValue=1;
    }
    ...
    var param = {
        ...
        indexName: indexName,
        indexData: indexData,
        visibleChild:visibleChildValue
    }

    var i = openProgressExt("保存中...");
    //return;
    $.ajax({
        url: ctx + '/platform/indexSave',
        data: param,
...
function drop(ev) {
    ...
    }else if(id == 'childBtn'){//子节点
        myDiagram.startTransaction("add child");// 刷新问题
        var childModel = {
            key: nodeDataArray.length,
            category: "8",      //表格
            text: "子节点",
            inServices: [{name: "input1", color: connectPoint}],
            outServices: [{name: "output1", color: connectPoint}]
        }
        myDiagram.model.addNodeData(childModel);
